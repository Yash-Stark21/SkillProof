...
**Status:** Accepted for MVP implementation<br>
**Target schema baseline:** `0.1`<br>
**Delivered migration head:** `0001_evidence_ledger`<br>
...
### 2.1 Staged schema delivery

The relationship overview is the approved MVP target, not a claim that every table exists in the first migration. Revision `0001_evidence_ledger` implements only `repositories`, `scans`, `repo_files`, and `evidence_items`, which is the minimum repository-to-evidence vertical slice. Sections 4.5 through 4.11 remain forward design for later additive migrations covering job parsing, matching, reports, scores, and claims.

All workflow categories in the delivered slice use readable `VARCHAR` columns with named `CHECK` constraints rather than native PostgreSQL enum types. UUIDv4 identifiers are generated in the application, and all schema creation/change occurs through Alembic.
...
All timestamps are `TIMESTAMPTZ` in UTC. Unless noted otherwise, foreign keys use `ON DELETE RESTRICT` to protect provenance. UUIDv4 values are generated by the application before persistence.
...
| `repository_snapshot` | JSONB | Required immutable API identity snapshot with `provider`, `identity`, `owner`, `name`, and `url` |
...
The serialized contract derives repository identity from the Scan snapshot, `commit_sha` and `coverage_state` from Scan, and `path` and `content_hash` from RepoFile/EvidenceItem so each API item remains self-contained without inconsistent duplicate storage. File hashes are calculated before text decoding/redaction; original line-ending bytes therefore remain significant.

Evidence serialization uses the immutable `scans.repository_snapshot` rather than mutable repository display metadata, so a later provider-side rename cannot rewrite historical identity. `claim_eligible` is derived as: owning scan is completed, evidence passes contract `0.1`, confidence is high or medium, and kind is not `documentation_reference`. Partial coverage does not invalidate positive observed evidence; it changes how absence is reported.
...
- **Run scan:** commit the running transition in a short transaction; perform network retrieval outside a write transaction; write the bounded file inventory, validated evidence, and completed state atomically in one final unit of work.
- **Fail scan persistence:** roll back the final unit completely, then write the safe failed state through a fresh session/transaction so partial evidence never appears complete.
...
- Alembic revision `0001_evidence_ledger` creates the four delivered tables, their `VARCHAR + CHECK` workflow constraints, foreign keys, uniqueness rules, and indexes in dependency order. Downgrade removes this slice in reverse order for disposable local/test use.
- Job-description, match, report, and claim tables are introduced only by later additive revisions after their vertical slices are implemented.
...
- The `0001` model/integration tests cover lifecycle checks, repository-upsert races and uniqueness, delete restrictions, file/hash invariants, semantic evidence deduplication, cross-scan rejection, immutable scan identity, and final-unit rollback. Later slices add revision, report, claim, and scoring tests with their migrations.
